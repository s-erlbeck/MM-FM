...
def _load_decoder_from_huggingface(repo_id: str, cache_dir: str = None):
    """
    Download Scale-RAE decoder from HuggingFace.

    Args:
        repo_id: HuggingFace repo ID (e.g., 'nyu-visionx/siglip2_decoder')
        cache_dir: Optional cache directory

    Returns:
        config_path: Path to downloaded config.json
        weights_path: Path to downloaded model.pt
    """
    ...
    print(f"Downloaded decoder from HuggingFace: {repo_id}")
...
class RAE(nn.Module):
    def __init__(self,
        ...
        decoder_from_hf: bool = False,  # Load decoder from HuggingFace
        decoder_drop_cls_token: bool = False,  # Whether decoder drops CLS token
        ...
        # Calculate base_patches: use encoder.num_tokens if available (for encoders that interpolate)
        # Otherwise calculate from input size and patch size
        if hasattr(self.encoder, 'num_tokens'):
            # Encoders like SigLIP2-SO400M explicitly set output token count
            self.base_patches = self.encoder.num_tokens
        else:
        ...
        # Store decoder_drop_cls_token for decode()
        self.decoder_drop_cls_token = decoder_drop_cls_token

        # Handle decoder loading: HuggingFace or local
        if decoder_from_hf:
            # Load decoder from HuggingFace (e.g., 'nyu-visionx/siglip2_decoder')
            hf_config_path, hf_weights_path = _load_decoder_from_huggingface(decoder_config_path)
            decoder_config = AutoConfig.from_pretrained(hf_config_path)
            # For HF decoder, hidden_size is already set in config (1152 for siglip2_decoder)
            # But we override to match encoder output just in case
            decoder_config.hidden_size = self.latent_dim
            # Calculate decoder image size from its patch_size
            # HF decoder (siglip2_decoder) has patch_size=14, so output = 14 * sqrt(256) = 224
            self.decoder_output_size = int(decoder_config.patch_size * sqrt(self.base_patches))
            decoder_config.image_size = self.decoder_output_size
            self.decoder = GeneralDecoder(decoder_config, num_patches=self.base_patches)
            # Load weights
            print(f"Loading pretrained decoder from HuggingFace: {decoder_config_path}")
            print(f"  Decoder output size: {self.decoder_output_size}x{self.decoder_output_size}")
            ...
            # Original local decoder loading
            ...
            self.decoder_output_size = int(decoder_patch_size * sqrt(self.base_patches))
            decoder_config.image_size = self.decoder_output_size
            self.decoder = GeneralDecoder(decoder_config, num_patches=self.base_patches)
    ...
    def encode(self, x: torch.Tensor, return_cls_token: bool = False):
        ...
        Args:
            ...
            return_cls_token: Whether to return CLS token (for GMM mode assignment in Stage 2)

        Returns:
            z: Spatial latents (B, C, H, W) if reshape_to_2d else (B, N, C)
            cls_token: Optional CLS token (B, C) if return_cls_token=True
        """
        # normalize input
        _, _, h, w = x.shape
        ...
        # Extract both spatial and CLS tokens
        z, cls_token = self.encoder(x)  # (B, N, C), (B, C)

        # Add noise to spatial tokens during training
        # NOTE: CLS token is NOT noised - it's used for GMM mode assignment
        ...
        if self.do_normalization:
            # Spatial: position-wise normalization
            ...
            # CLS: No additional normalization needed
            # DINOv2 already layer-normalizes CLS token
    ...
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        ...
        output = self.decoder(z, drop_cls_token=self.decoder_drop_cls_token).logits
