...
from .gmm_sampler import GMMSampler
...
class Transport:
    ...
    def sample(self, x1, y=None, y_gmm=None):
        """Sampling x0 & t based on shape of x1 (if needed)
          Args:
            x1 - data point; [batch, *dim]
            y - class/mode labels for model; [batch] (optional)
            y_gmm - GMM mode indices for noise sampling; [batch] (optional)
                   If None, falls back to y for backward compatibility.
        """
        batch_size = x1.shape[0]

        # Determine which labels to use for GMM sampling
    ...
    def training_losses(
        ...
        - model_kwargs: additional arguments for the model
            - y: labels for model conditioning (ImageNet classes or GMM modes)
            - y_gmm: labels for GMM noise sampling (optional, defaults to y)
        - reduction: 'mean' (scalar) or 'none' (per-sample for adaptive weighting)
        """
        if model_kwargs == None:
            model_kwargs = {}

        # Extract labels for GMM sampling and model conditioning
        # y_gmm: used for GMM noise sampling (mode indices)
        # y: used for model conditioning (class or mode labels)
        # Pop y_gmm to avoid passing it to the model forward call
        y_gmm = model_kwargs.pop('y_gmm', None)
        y = model_kwargs.get('y', None)

        t, x0, x1 = self.sample(x1, y=y, y_gmm=y_gmm)
        ...
        if self.model_type == ModelType.VELOCITY:
            if reduction == 'none':
                # Per-sample loss: (B,)
                terms['loss'] = ((model_output - ut) ** 2).mean(dim=list(range(1, len(model_output.shape))))
            else:
