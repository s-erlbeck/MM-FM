...
class DiTwDDTHead(nn.Module):
    ...
    def forward(self, x, t, y=None, s=None, mask=None):
        ...
        if self.use_y_conditioning:
            if y is None:
                raise ValueError("y is required when use_y_conditioning=True")
            y = self.y_embedder(y, self.training)
            c = nn.functional.silu(t + y)
        else:
            # Unconditional: c = silu(t)
        ...
        if s is None:
            ...
            # broadcast t to match s: (B, S, D)
            t = t.unsqueeze(1).expand(-1, s.shape[1], -1)
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale, cfg_interval=(0, 1)):
        """
        Forward pass with classifier-free guidance, following Scale-RAE exactly.

        For class-conditional / mode-conditional:
            y: (2B,) pre-concatenated [y_cond, y_null] integer labels.
        """
        if not self.use_y_conditioning:
            raise ValueError(
                "forward_with_cfg() requires use_y_conditioning=True. "
                "For unconditional models, use forward() directly."
            )

        # Class-conditional / mode-conditional: y is pre-concatenated [y_cond, y_null] of size 2B
        half = x[: len(x) // 2]
