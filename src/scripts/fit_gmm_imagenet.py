...
def main():
    ...
    cls_tokens = None
    data_limited = args.data_limit_sample_percentage < 1.0 or args.data_limit_class_percentage < 1.0
    if data_limited:
        cls_tokens_path = args.cls_tokens_path or os.path.join(
            args.output_dir,
            f'cls_tokens_p{args.data_limit_sample_percentage}_c{args.data_limit_class_percentage}_s{args.data_limit_seed}.npz',
        )
    else:
